...
const NewsContent = (props: { title: string, modalTitle: string, content: string, link: string, imgpass: string }) => {
    ...
    return (
        <Box>
            <Box className='H1' onClick={handleOpen} sx={{ maxWidth: '800px', margin: { xs: '3% auto', sm: '3% auto' }, justifyContent: 'center', alignItems: 'center', cursor: 'pointer' }}>
                <Box sx={{ margin: '0 7%', textAlign: 'left' }}>
                    <Box fontWeight="fontWeightLight" sx={{ fontSize: { xs: '12px', sm: '16px' }, padding: '5px 0 5px 0' }}>
                        {props.title}
                    </Box>
                </Box>
            </Box>
